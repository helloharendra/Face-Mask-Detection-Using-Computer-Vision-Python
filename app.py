...
from configu import *
from detect_mask_image import detect
from db import Video, Image, init_db
from sqlalchemy.orm import sessionmaker
from sqlalchemy import create_engine
import cv2
from detect_mask_video import video
...
if choice == 'About project':
    ...
    st.image('image/maskgirls.png')
    st.info('''Face Mask Detection Platform utilizes Artificial Network to perceive if a person does/doesn't wear a mask.
The application can be associated with any current or new cameras to identify individuals with/without a mask.
By developing a face mask detection technique,
through which we identify people weared mask or not.
so we suggest people who not weared mask to wear it to reduce the covid/ any viral disease spread''')
...
if choice == 'image based test':
    ...
    if img:
        try:
            ...
            file_extension = img.type.split('/')[1]
            filename = f"{img.name.split('.')[0]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{file_extension}"
            path = os.path.join("images", filename)
            
            # Save the uploaded file
            im.save(path, format=file_extension.upper())
            
            # Save to database
            status = save_image(path)
            ...
            if status:
                ...
                processed_path = os.path.join("images", processed_filename)
                cv2.imwrite(processed_path, out_img)
                
                col2.image(processed_path, use_column_width=True, caption='Prediction')
                st.success("Mask detection completed!")
            else:
                st.sidebar.error('Upload failed')
...
if choice == 'realtime detection':
    ...
    cnf = st.slider('confidence threshold', min_value=0.1, max_value=1.0, value=0.5)
    btn = st.button("start camera window")
    st.info('Click on start camera window for detection')
    
    if btn:
        try:
            video(cnf=cnf)
        except Exception as e:
            st.error(f"Error starting video detection: {e}")
...
if choice == 'view previous predictions':
    try:
        ...
        if results:
            ...
            for img in results:
                st.write(f"**Image:** {img.filename}.{img.extension}")
                st.write(f"**Uploaded:** {img.created_on}")
                st.write(f"**Path:** {img.filepath}")
                if os.path.exists(img.filepath):
                    st.image(img.filepath, width=300)
                st.divider()
        else:
        ...
        st.error(f"Error loading previous predictions: {e}")